...
public class Main {
	...
	public static String drumXMLParser(ArrayList<DrumNote> drumNoteArray) {
		...
		directives.add("score-partwise").attr("version", "3.0").add("work").add("work-title").set(tabTitle).up().up()
				.add("identification").add("creator").attr("type", "composer").set(tabComposer).up().up()
				.add("part-list").add("score-part").attr("id", "P1").add("part-name").set("Drumset")
		...
		for (int i = 0; i < 2; i++) {
			doOnce = 0;
			directives.add("measure").attr("number", i + 1).add("attributes").add("divisions").set("4").up()// divisions
					.add("key").add("fifths").set(0).up().up()

					.add("time").add("beats").set(4).up().add("beat-type").set(4).up().up()

					.add("clef").add("sign").set("percussion").up().add("line").set(2).up().up()
			...
			for (DrumNote drumNote : drumNoteArray) {
				...
				if (drumNote.measure == i + 1) {
					...
					if (voiceChange != 0) {
						directives.add("backup").add("duration").set(16).up().up();
					}

					directives.add("note").add("unpitched");
					...
					if (drumNote.part.equalsIgnoreCase("SD") || drumNote.part.equalsIgnoreCase("S")) {
						directives.set("C");
					} else if (drumNote.part.equalsIgnoreCase("BD") || drumNote.part.equalsIgnoreCase("B")) {
						directives.set("F");
					} else if (drumNote.part.equalsIgnoreCase("HH") || drumNote.part.equalsIgnoreCase("H")
							|| drumNote.part.equalsIgnoreCase("HT")) {
						directives.set("G");
					} else if (drumNote.part.equalsIgnoreCase("RD") || drumNote.part.equalsIgnoreCase("R")) {
						directives.set("F");
					} else if (drumNote.part.equalsIgnoreCase("CC") || drumNote.part.equalsIgnoreCase("C")) {
						directives.set("A");
					} else if (drumNote.part.equalsIgnoreCase("ST") || drumNote.part.equalsIgnoreCase("T1")) {

						directives.set("E");
					} else if (drumNote.part.equalsIgnoreCase("MT") || drumNote.part.equalsIgnoreCase("T2")) {

						directives.set("D");
					} else if (drumNote.part.equalsIgnoreCase("FT") || drumNote.part.equalsIgnoreCase("T3")) {

						directives.set("A");
					} else {

						// Error error = new Error("Tablature incorrect format. No specified drum
						// instrument! (Example: BD|--x-x-| is correct. |--x-x-| is not.)");
					}

					directives.up().add("display-octave");
					if (drumNote.part.equalsIgnoreCase("SD") || drumNote.part.equalsIgnoreCase("S")) {
						directives.set(5);
					} else if (drumNote.part.equalsIgnoreCase("BD") || drumNote.part.equalsIgnoreCase("B")) {
						directives.set(4);
					...
					if (drumNote.backUp == false) {
						directives.add("voice").set(1).up();
					} else {
						directives.add("voice").set(2).up();
					}

					directives.add("type").set("eighth").up();

					if (drumNote.backUp == false) {
						directives.add("stem").set("up").up();
					} else {
						directives.add("stem").set("down").up();
					}

					if (drumNote.noteValue == 'o') {

					} else {
						directives.add("notehead").set(drumNote.noteValue).up();
					}

					directives.add("beam").attr("number", "1")
